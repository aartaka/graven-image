...
- [ ] Test on more implementations. I'm currently testing it on SBCL,
  CCL, ECL, ABCL, CLISP, and GCL. Will be glad to accept patches/bug
  reports for other implementations.
...
- [ ] Re-use the implementation-specific command processing instead of
  =definterface=.
  - [ ] CCL has =ccl:define-toplevel-command=.
  - [ ] SBCL only has debug/step commands via =!def-debug-command=,
    =!def-debug-command-alias=, =!def-step-command=.
  - [ ] ABCL has =*top-level:command-table*=
  - [ ] CLISP with =*user-commands*=.
  - [ ] ECL's =*tpl-commands*=.
  - [ ] Allegro =TPL::ADD-NEW-COMMAND= and =TPL::*COMMAND-HASH-TABLE*= and a whole [[https://franz.com/support/documentation/current/doc/top-level.htm#adding-new-commands-1][11.0 Adding New Commands]] section in the manual page on the Top Level.
